...
export function generateValue(
  ...
  // Support AUTO: prefix – used to mark fields that should be auto-generated.
  // The prefix controls behavior at a higher level (when to generate), not the pattern itself.
  if (!generationRule) return "";
  const rule = generationRule.startsWith("AUTO:")
    ? generationRule.slice(5)
    : generationRule;
  ...
  for (let i = 0; i < patterns.length; i++) {
    ...
    if (pattern.startsWith('(') && pattern.endsWith(')')) {
      ...
      // This is static text (prefix/suffix). Replace underscores with spaces (DSL code for space).
      parts.push({ type: 'static', value: pattern.replace(/_/g, ' ') });
    }
  }
  ...
  // Support escaped newlines (\\n) for multi-line QR codes and text
...
function extractFromField(pattern: string, allFields: Record<string, string | number | boolean>): string {
  ...
  const fallbackKey = pattern.replace(/^dep_/, '');
  const fallbackValue = String(allFields[pattern] || allFields[fallbackKey] || '');
...
export function generateAutoFields(fields: FormField[], isPurchased: boolean = false): FormField[] {
  ...
  // Generate AUTO fields — fields whose generationRule starts with "AUTO:"
  // (set via the AUTO toggle in the admin GenRuleBuilder)
  const autoGeneratedValues: Record<string, string> = {};
  
  const autoFields = fields.filter((field) => {
    const isAutoRule = field.generationRule?.startsWith("AUTO:") || 
                       field.generationMode === "auto" ||
                       ((field.type === "qrcode" || field.type === "barcode" || field.isTrackingId) && field.generationRule);
    return isAutoRule;
  });
  ...
  autoFields.forEach((field) => {
      ...
      // Update allFieldValues to allow chaining dependencies
      allFieldValues[field.id] = generated;
    });

  // Return fields with AUTO values set
  return fields.map((field) => {
    const isAutoRule = field.generationRule?.startsWith("AUTO:") || 
                       field.generationMode === "auto" ||
                       ((field.type === "qrcode" || field.type === "barcode" || field.isTrackingId) && field.generationRule);
    
    if (isAutoRule && autoGeneratedValues[field.id] !== undefined) {
      return {
        ...field,
        currentValue: autoGeneratedValues[field.id],
      };
    }
    return field;
  });

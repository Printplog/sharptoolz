...
export default function SvgFormTranslator({ isPurchased, templateId: templateIdProp }: Props) {
  ...
  // EFFECT 4: Live Preview Update - Re-runs whenever fields or SVG content changes
  useEffect(() => {
    ...
    let newSvgText = updateSvgFromFormData(svgContent, fieldsWithAutoGenerated);

    const finalizeSvg = async () => {
      if (data.fonts && data.fonts.length > 0) {
        newSvgText = await injectFontsIntoSVG(newSvgText, data.fonts, BASE_URL, true);
      }

      // Inject images (including signatures/blobs) for preview accuracy
      newSvgText = await injectImagesIntoSVG(newSvgText, BASE_URL);

      setSvgText(newSvgText);
      setSvgRaw(newSvgText); // Clean SVG for download (Image rendering doesn't need ID namespacing)

      // Sanitize gradients ONLY for the on-page preview (FIXES id collisions in DOM)
      const ns = svgNamespace(newSvgText);
      const sanitizedBase = sanitizeSvgGradients(newSvgText, ns);

      // Apply watermark ONLY for the on-screen preview
    ...
    finalizeSvg();
  }, [svgContent, isSvgFetching, data, fields, isPurchased, setSvgRaw]);
  ...
  // SIMPLIFIED: Update preview whenever fields change.
  // We run this regardless of activeTab so that svgRaw (the download source) is ALWAYS
  // mathematically up-to-date with the latest editor changes, eliminating stale downloads.
  useEffect(() => {
    if (!baseSvgRef.current || !fields || fields.length === 0) return;

    const updateSimplifiedPreview = async () => {
      const freshFields = useToolStore.getState().fields;
      const fieldsWithAutoGenerated = generateAutoFields(freshFields, isPurchased);
      let updatedSvg = updateSvgFromFormData(baseSvgRef.current, fieldsWithAutoGenerated);

      // Inject images (including signatures/blobs) for preview accuracy
      updatedSvg = await injectImagesIntoSVG(updatedSvg, BASE_URL);

      // Update global store with the "clean" SVG (no namespacing needed for download/image path)
      setSvgRaw(updatedSvg);

      // Sanitize gradients for the PREVIEW tab (where it's injected inline)
      const ns = svgNamespace(updatedSvg);
      const sanitized = sanitizeSvgGradients(updatedSvg, ns);

      const shouldWatermark = !isPurchased || (isPurchased && (data as { test?: boolean } | undefined)?.test === true);
      const previewSvg = shouldWatermark ? addWatermarkToSvg(sanitized) : sanitized;
      setLivePreview(previewSvg);
    };

    updateSimplifiedPreview();

  }, [fields, activeTab, isPurchased, data]);
        ...
        onValueChange={(value) => {
          ...
          if (value === "preview" && baseSvgRef.current) {
            ...
            if (freshFields.length > 0) {
              ...
              const shouldWatermark = !isPurchased || (isPurchased && (data as { test?: boolean } | undefined)?.test === true);
              setLivePreview(shouldWatermark ? addWatermarkToSvg(updatedSvg) : updatedSvg);
            }
          }
        }}
      >

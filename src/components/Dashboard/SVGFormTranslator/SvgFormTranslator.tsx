...
export default function SvgFormTranslator({ isPurchased, templateId: templateIdProp }: Props) {
  ...
  useEffect(() => {
    ...
    const finalizeSvg = async () => {
      ...
      // - Non-purchased template (browsing /tools/:id): always show watermark
      // - Purchased template (/documents/:id): only show if test === true
      // - Admin editor uses AdminSvgFormTranslator (separate) — no watermark there
      const shouldWatermark = !isPurchased || (isPurchased && (data as { test?: boolean }).test === true);
      const previewSvg = shouldWatermark ? addWatermarkToSvg(newSvgText) : newSvgText;

      setSvgText(newSvgText);
      setSvgRaw(newSvgText);
      const ns = svgNamespace(previewSvg);
      setLivePreview(sanitizeSvgGradients(previewSvg, ns));
  ...
  useEffect(() => {
    ...
    const out = shouldWatermark ? addWatermarkToSvg(updatedSvg) : updatedSvg;
    setLivePreview(sanitizeSvgGradients(out, svgNamespace(out)));

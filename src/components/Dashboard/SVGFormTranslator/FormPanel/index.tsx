...
const FormPanel = React.memo(function FormPanel({
  ...
  const createDocument = async (isTest: boolean) => {
    ...
    if (!isPurchased) {
      (fields ?? [])
        .filter((field) =>
          // Generate if it has AUTO: prefix OR if it is a Tracking ID with a rule OR a QR code with a rule
          field.generationRule?.startsWith("AUTO:") ||
    ...
    const fieldUpdates: FieldUpdate[] =
      (fields ?? [])
        ...
        .map((field) => {
          ...
          const isAutoRule = field.generationRule?.startsWith("AUTO:") || (field.type === "qrcode" && field.generationRule);
          if (!isPurchased && isAutoRule && autoGeneratedValues[field.id] !== undefined) {

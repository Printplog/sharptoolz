...
export default function ReferralLink({ link, isLoading, rewardPercentage, minDeposit }: Props) {
  ...
  const shareMessage = `Check out SharpToolz! I'm earning rewards by generating professional documents. Join me here: ${link}`;
  ...
  const shareOptions = [
    {
      ...
      onClick: () => window.open(`https://wa.me/?text=${encodeURIComponent(`${shareMessage} Link: ${link}`)}`, '_blank'),
      isSvg: true
    },
    {
      name: "X (Twitter)",
      ...
      onClick: () => window.open(`https://x.com/intent/tweet?text=${encodeURIComponent(shareMessage)}&url=${encodeURIComponent(link || "")}`, '_blank'),
      isSvg: true
    },
    {
      name: "Facebook",

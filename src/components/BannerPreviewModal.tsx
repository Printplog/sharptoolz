...
export function BannerPreviewModal({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent
                className="max-w-5xl p-0 overflow-hidden bg-transparent border-none shadow-none focus:outline-none"
                style={{ maxHeight: "95vh" }}
            >
                <DialogTitle className="sr-only">{templateName} Preview</DialogTitle>
                <div className="relative bg-[#0a0a0a] rounded-3xl border border-white/10 overflow-hidden max-h-[95vh] flex flex-col">
                    <button
                        onClick={onClose}
